...
Server endpoints verify JWTs using `decodeAndVerifyJWT` from `@zconsole/party-pics-shared`. The function validates the signature, expiration, and returns the typed payload. See `/shared/src/jwt.ts` for implementation.
