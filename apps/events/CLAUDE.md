...
- `/docs/` - Documentation
  ...
  - `secrets.md` - How secrets are managed locally (`.env` + the `[secrets]` gate) and in production (Worker secret bindings via Alchemy)
...
Server endpoints verify JWTs using `decodeAndVerifyJWT` from `@zconsole/events-shared`. The function validates the signature, expiration, and returns the typed payload. See `/shared/src/jwt.ts` for implementation.
