...
- `/docs/` - Documentation
  ...
  - `secrets.md` - How secrets and env vars are managed locally (`.env`) and in production (Alchemy)
...
Server endpoints verify JWTs using `decodeAndVerifyJWT` from `@zconsole/console-shared`. The function validates the signature, expiration, and returns the typed payload. See `/shared/src/jwt.ts` for implementation.
